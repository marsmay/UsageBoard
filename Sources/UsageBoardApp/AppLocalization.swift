...
struct AppLocalization {
    ...
    func text(_ key: Key) -> String {
        ...
        case (.restartRequiredTitle, .en): return "Apply the new language now?"
        case (.restartRequiredTitle, .zhHans): return "立即应用新语言？"
        case (.restartRequiredMessage, .en): return "Restart UsageBoard to use the new language. You can also restart later; your language selection will be kept."
        case (.restartRequiredMessage, .zhHans): return "重启 UsageBoard 后，界面将使用新语言。也可以稍后重启，所选语言会保留。"
        case (.restartNow, .en): return "Restart Now"
        case (.restartNow, .zhHans): return "立即重启"
