...
- 选择不同于当前运行语言的选项时，保存设置并通过 SwiftUI 状态呈现重启提示；“立即重启”通过 `AppRelauncher.relaunchCurrent()` 重启，退出前等待配置保存完成；“稍后重启”保留所选语言，当前界面保持不变。切回当前运行语言不弹窗。
...
- Menu bar popover：宽度固定 380，高度随当前标签内容及图表展开/收起调整，最大为状态栏所在屏幕可用高度的 75%；顶层使用纵向固有尺寸约束，使窗口可在内容变少时收缩。
